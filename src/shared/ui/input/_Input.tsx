...
type IconType = 'eye-on' | 'eye-off' | null;
...
const Input = ({ type, error, disabled, label, ...rest }: Props) => {
    ...
    const [currentInputType, setСurrentInputType] = useState<InputTypes>(type);

    const [Icon, setIcon] = useState<IconType>('eye-on');
    ...
    const handleChangeCurrentType = () => {
        switch (currentInputType) {
            case 'password': {
                setIcon('eye-off');
                setСurrentInputType('text');
                break;
            }

            case 'text': {
                setIcon('eye-on');
                setСurrentInputType('password');
                break;
            }
        }
    };
